import { pgTable, serial, integer, text, real, timestamp, boolean, pgEnum, uniqueIndex } from "drizzle-orm/pg-core";
...
export const cookCheckins = pgTable(
  "cook_checkins",
  {
  ...
  (table) => [
    uniqueIndex("cook_checkins_cook_client_operation_unique")
      .on(table.cookId, table.clientOperationId),
  ],
);
