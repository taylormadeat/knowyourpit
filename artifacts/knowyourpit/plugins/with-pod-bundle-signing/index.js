...
const APPCHECK_MARKER = "# PIT_APPCHECK_MODULAR_HEADERS";
const REVENUECAT_PIN_MARKER = "# PIT_REVENUECAT_PIN";
const MIN_IOS = "16.1";

const withPodBundleSigning = (config) => {
  // Note: do NOT guard on config.platform here — at app.config.js evaluation time
  // config.platform is undefined (not "ios"), so the guard would always short-circuit.
  // Platform filtering is handled by withDangerousMod's ["ios", ...] tuple below.
  return withDangerousMod(config, [
    ...
    async (cfg) => {
      ...
      const alreadyPatched =
        ...
        contents.includes(APPCHECK_MARKER);
      if (alreadyPatched) {
