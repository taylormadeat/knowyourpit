 ...
 * service is classified into one of the known thermometer device types.
 * The caller passes those hosts to the matching polling adapter instead of
...
const KNOWN_TYPES: ZeroconfDeviceType[] = ["fireboard"];
...
export function useZeroconfDiscovery(enabled: boolean): UseZeroconfDiscoveryResult {
  ...
  // Initialise the native browser instance once
  useEffect(() => {
    ...
    const onRemoved = (service: ZeroconfService) => {
      if (!mountedRef.current) return;
      const ipv4 = service.addresses?.find((a) => /^\d{1,3}(\.\d{1,3}){3}$/.test(a));
      const host = ipv4 ?? service.host;
      const type = classifyService(service.name, service.host, service.port);
