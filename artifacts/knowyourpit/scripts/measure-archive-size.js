 ...
 * ─── TOP-N LARGEST FILES ──────────────────────────────────────────────────
 *
 * After the size summary the script prints the largest files in the archive
 * so bloat is easy to diagnose when the total jumps unexpectedly.
 *
 * Default: top 10 files.  Override with --top:
 *
 *   node scripts/measure-archive-size.js --top 20
 *   node scripts/measure-archive-size.js --top 5 --warn-threshold 10%
 *
 * ─── STANDALONE ───────────────────────────────────────────────────────────
 *
...
/**
 * Parse --top <n> from process.argv.
 * Returns the number of files to list (default 10).
 */
function parseTopFlag(argv) {
  const idx = argv.indexOf("--top");
  if (idx === -1) return 10;

  const raw = argv[idx + 1];
  if (!raw) {
    console.error("ERROR: --top requires a numeric value (e.g. --top 20)");
    process.exit(1);
  }

  const n = parseInt(raw, 10);
  if (isNaN(n) || n <= 0) {
    console.error(`ERROR: --top must be a positive integer, got "${raw}"`);
    process.exit(1);
  }
 ...
 * Returns { bytes, files, fileList } where fileList is an array of
 * { rel, bytes } objects for every included file.
 */
function walk(dir, relBase, compiled) {
  ...
  try {
    ...
    return { bytes: 0, files: 0, fileList: [] };
  }
  ...
  for (const entry of entries) {
    ...
    if (isDir) {
      ...
      bytes += sub.bytes;
      files += sub.files;
      for (const f of sub.fileList) fileList.push(f);
    } else if (entry.isFile()) {
      try {
        const size = fs.statSync(full).size;
        bytes += size;
        files++;
        fileList.push({ rel, bytes: size });
      } catch {
...
function printTopFiles(fileList, topN) {
  const sorted = fileList.slice().sort((a, b) => b.bytes - a.bytes);
  const top = sorted.slice(0, topN);

  console.log(`  Top ${top.length} largest files:`);
  console.log();

  // Determine column widths for neat alignment
  const maxSizeLen = Math.max(...top.map((f) => humanSize(f.bytes).length), 4);
  for (let i = 0; i < top.length; i++) {
    const rank = String(i + 1).padStart(2);
    const size = humanSize(top[i].bytes).padStart(maxSizeLen);
    console.log(`    ${rank}.  ${size}  ${top[i].rel}`);
  }
...
function main() {
  ...
  const topN = parseTopFlag(process.argv);
  ...
  const { bytes, files, fileList } = walk(projectRoot, "", compiled);
