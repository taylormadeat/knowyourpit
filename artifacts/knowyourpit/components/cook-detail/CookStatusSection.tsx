...
import { PreviewSurface } from "@/components/preview/PreviewPrimitives";
...
export function CookStatusSection({
  ...
  return (
    <>
      {cookStatus !== "active" && (
        <View style={[{ flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 12, paddingVertical: 8, borderRadius: isPreview ? 12 : colors.radius, borderWidth: 1, borderColor: isPreview ? colors.border : statusColor + "40", backgroundColor: isPreview ? colors.card : statusColor + "18" }]}>
          ...
          {(() => {
            ...
            return <View style={{ flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: colors.background, borderRadius: 6, paddingHorizontal: 7, paddingVertical: 3, borderWidth: 1, borderColor: colors.border, marginLeft: 4 }}><Text style={{ fontFamily: "Inter_600SemiBold", fontSize: 11, color: colors.foreground }}>{sizeText}</Text></View>;
          })()}
      ...
      {c.status === "completed" && c.isOutlier && !c.outlierDismissed && (
        <View style={{ borderRadius: isPreview ? 12 : colors.radius, backgroundColor: "#f59e0b12", borderWidth: 1, borderColor: "#f59e0b40", paddingHorizontal: 14, paddingVertical: 11, gap: 8 }}>
      ...
      {cookStatus === "planned" && !!(c as any).fromFrozen && !!(c as any).actualThawStartAt && (
        <Pressable onPress={() => router.push(`/(tabs)/plan?replanCookId=${id}` as any)} style={({ pressed }) => ({ flexDirection: "row" as const, alignItems: "center" as const, justifyContent: "center" as const, gap: 6, borderWidth: 1, borderColor: "#38bdf8", borderRadius: isPreview ? 12 : colors.radius, paddingVertical: 10, marginTop: 4, opacity: pressed ? 0.7 : 1 })}>
