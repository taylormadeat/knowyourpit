...
import { getCookSuggestions } from "@/utils/getCookSuggestions";
...
export function CookModals({
  ...
  return (
    <>
      ...
      <AddToPlannedCookModal visible={addToSessionOpen} onClose={() => setAddToSessionOpen(false)} colors={colors} insets={insets} cookId={Number(id)} cookFoodType={(cook as any)?.foodType ?? null} cookWeightLbs={(cook as any)?.weightLbs ?? null} cookGrillId={(cook as any)?.grillId ?? null} cookSnapshot={cook as any} grills={grills} onSuccess={() => {}} />
