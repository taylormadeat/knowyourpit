...
export function LiveCookSection(p: Props) {
  ...
  const otherInkbirdProbes = React.useMemo(() => {
    // Pin the last-known device to the top for convenience.
    if (!lastKnownInkbirdDeviceId) return sortedInkbirdProbes;
    const lastIdx = sortedInkbirdProbes.findIndex((p) => p.deviceId === lastKnownInkbirdDeviceId);
    if (lastIdx <= 0) return sortedInkbirdProbes;
    const reordered = [...sortedInkbirdProbes];
    const [pinned] = reordered.splice(lastIdx, 1);
    reordered.unshift(pinned);
    return reordered;
  }, [sortedInkbirdProbes, lastKnownInkbirdDeviceId]);
  const otherBleContextDevices = sortedBleContextDevices;
