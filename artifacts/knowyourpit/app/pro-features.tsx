  ...
  return (
    <View style={[s.container, { backgroundColor: colors.background }]}>
      ...
      >
        {/* Hero */}
        <LinearGradient
          colors={isPreview ? [colors.card, colors.background] : ["#2D1A0E", "#1C1C1F"]}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
          style={[s.hero, isPreview && { borderBottomWidth: 1, borderBottomColor: colors.border }]}
        >
          <View style={[s.heroBadge, isPreview && { backgroundColor: colors.primary + "20" }]}>
            <Feather name="award" size={13} color={colors.primary} />
            <Text style={[s.heroBadgeText, { color: colors.primary }]}>knowyourpit PRO</Text>
          </View>
          <Text style={[s.heroTitle, isPreview && { color: colors.foreground }]}>Everything you need to cook like a pro.</Text>
          <Text style={[s.heroSub, isPreview && { color: colors.mutedForeground }]}>One subscription unlocks all of these features — including unlimited cooks, photo scans, and cook history.</Text>
        </LinearGradient>
