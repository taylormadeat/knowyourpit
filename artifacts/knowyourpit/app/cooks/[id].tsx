...
export default function CookDetailScreen() {
  ...
  return (
    <View style={[s.container, { backgroundColor: colors.background }]}>
      ...
      <LinearGradient colors={isPreview ? [colors.background, colors.background] : ["#1C1C1F", "#2D1A0E"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[s.header, { paddingTop: topPad + 14, borderBottomWidth: isPreview ? 1 : 0, borderBottomColor: colors.border }]}>
        <LogoBackground opacity={0.06} />
        <Pressable onPress={goBack} style={s.backBtn}><Feather name="chevron-left" size={24} color={isPreview ? colors.foreground : "#F3EDE1"} /></Pressable>
        <Text style={[s.headerTitle, { color: isPreview ? colors.foreground : "#F3EDE1" }]} numberOfLines={1}>{c.foodType || "Cook"}</Text>
        <View style={s.headerRight}>
          {cookStatus === "active" && <Pressable onPress={() => setEditTimesVisible(true)} style={[s.editBtn, { marginRight: 2 }]} hitSlop={8}><Feather name="clock" size={17} color={isPreview ? colors.foreground : "#F3EDE1"} /></Pressable>}
          <Pressable onPress={openEdit} style={s.editBtn} hitSlop={8}><Feather name="edit-2" size={17} color={isPreview ? colors.foreground : "#F3EDE1"} /></Pressable>
      ...
      {!isPreview && <View style={s.fireBar} />}
