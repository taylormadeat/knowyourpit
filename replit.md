...
- **2026-06-07 — Build #119 / v1.0.17 queued to TestFlight**: EAS build id `8543de8b-0aa5-412f-b5e8-7e7d88efa0a8`. buildNumber 118→119, version stays 1.0.17. Fixes: UI tap freeze (Plan/Multi-Cook/Cook Now buttons open loading modal instantly); Connected Devices spinner freeze (MEATER + ThermoWorks status queries use staleTime + keepPreviousData); ThermoWorks phantom channel probes (switched isChannelLive from denylist to allowlist, collection query replaces 1-8 individual Firestore reads, freshness window tightened 15 min → 5 min). No DB schema changes.
