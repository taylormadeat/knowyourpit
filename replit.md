...
- **2026-06-15 — Android Build #1 / v1.0.17 queued**: EAS build id `32e661e0-37df-4c2c-9272-b089735bf328`. Android production AAB, versionCode 4. Fixes included: multi-cook AI baseline anchoring, home screen cache invalidation on delete, Android Gradle `META-INF/versions/9/OSGI-INF/MANIFEST.MF` packagingOptions fix. Manual upload to Play Console required (Google Cloud org policy blocks service account JSON key creation). Logs: https://expo.dev/accounts/taylormadeat/projects/knowyourpit/builds/32e661e0-37df-4c2c-9272-b089735bf328. Note: `.git/` added to `.easignore` — it was 409 MB and the dominant contributor to archive size (compressed to ~195 MB); excluding it is safe since EAS_NO_VCS=1 already bypasses git.
