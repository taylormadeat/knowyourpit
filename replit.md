...
- **2026-06-14 — Builds #124–#129 / v1.0.17 all errored** (AppCheckCore blocker, unresolved): EAS build IDs `b54eeddb`, `dd0dc5eb`, `f0eebd4b` (plus earlier `aa3ad42f`, `7a868ab1`, `33b3feb5`, `165d924c`). All fail in the "Install pods" phase. Root cause: `AppCheckCore` (Swift pod, source pod unknown — NOT `@react-native-google-signin` which is not in this project) requires `GoogleUtilities` and `RecaptchaInterop` to expose module maps. CocoaPods validation fires **before** `post_install` and **before** the Xcode project is generated, so `DEFINES_MODULE=YES` in `post_install` does not satisfy it. Attempted fixes and why they fail: (1) `use_modular_headers!` inside target — conflicts with `use_react_native!` or similar RN pod integration, causes faster/earlier failure; (2) `pod 'GoogleUtilities', :modular_headers => true` + `pod 'RecaptchaInterop', :modular_headers => true` inside target — also causes earlier failure, possibly a resolution conflict; (3) `use_modular_headers!` globally (before target block) — conflicts with React Native pods (boost, Folly, etc.), fastest failure. Build `165d924c` ran pod install to line 302-305 (near-completion) with ONLY `DEFINES_MODULE` in `post_install` — this is the baseline that gets furthest. **Next action required**: must read the actual EAS build log in the dashboard (Install pods phase) for `f0eebd4b` to see the exact failure line; then run `pod install` locally on a Mac to test fixes interactively. Podfile and plugin have been restored to the `165d924c` baseline (no pre-install modular headers changes). Cache key left at v32. The source pod pulling in AppCheckCore must be identified from the local `Podfile.lock`.
