...
- **KCBS Competition Mode (Pro)**: Pro-gated workflow on the Plan tab for backwards-planning a 4-category KCBS competition (chicken, ribs, pork, brisket) to per-item turn-in times. The server deterministically recomputes `grillLightAt` / `meatOnAt` / `estimatedFinishAt` from each item's `turnInAt` (finish = turnIn − 15min rest-buffer − restMinutes; meatOn = finish − duration; light = meatOn − preheat) and emits a per-item `warning` when a back-planned start is already in the past or unrealistically tight. The session view renders a competition badge, category chips, real-time countdowns to each turn-in (red inside the last 30 min), a 15-minute box-packing step, and a "Log Your Results" sheet for placement / judge score / notes. Competition placements feed the PitMaster Score with the highest weight (0.5). The Cook Log includes a "Competitions" filter chip.
