...
- **Temperature Monitoring**: Supports uploading data from various Bluetooth temperature probes (Inkbird, Govee, and others) and displays historical data with charts.
...
- **Inkbird, Govee**: External Bluetooth temperature probe brands supported for data upload.
...
- **2026-08-16 — Big Pete's multi-cook fix + badge rename — OTA iOS only**: Two post-launch fixes OTA-pushed to iOS production (update group `c3dfc505`). (1) `isMultiCook` flag in `BigPetesSeasoningCard` was only `true` when `distinctCategories.size > 1`; changed to always `true` when the `cuts` prop is provided so multi-cook copy ("Perfect pairings for your multi-cook") renders correctly even for same-category item lists. (2) `DEFAULT_CONFIG.partnerBigPetes` changed `false` → `true` so the card renders immediately on first load and survives any API timeout — matching the stated intent in the original comment. (3) Added `if (!current) return null` null guard before `current.accent` access in case `pairings[idx]` is transiently undefined during a cuts-change render. (4) Badge text changed from `"🤝 FEATURED PARTNER"` → `"🤝 Big Pete's Seasoning"` in both `BigPetesHomeCard` and `BigPetesSeasoningCard`. Android OTA paused by user request — not pushed.

- **2026-08-16 — Build #135 (iOS) / versionCode 5 (Android) / v1.0.18 — Big Pete's Seasoning partnership launch**: EAS iOS `b443fb6f-465e-4268-949b-bdababd2fb7c`, Android `8d2a829d-0ca6-40e1-9c63-1ba8e1d76e84`. Both submitted 2026-08-16. First build shipping the Big Pete's Seasoning partnership feature. New components: `BigPetesHomeCard` (Home tab promotional card with product pairings carousel) and `BigPetesSeasoningCard` (Plan screen card, handles single-cook and multi-cook modes). Controlled by remote feature flag `partnerBigPetes` in `useRemoteConfig`; toggle via `POST /api/admin/config {"key":"partnerBigPetes","value":false}` + `Authorization: Bearer <ADMIN_API_TOKEN>`. Server-side flag defaults `true` unless `FEATURE_PARTNER_BIG_PETES=false` env var is set. Six seasoning SKUs with curated meat pairings baked into `PAIRINGS` constant.
...
- **2026-06-27 — WiFi probe detection — code review complete, hardware test PENDING** (WiFi probe cloud integrations later removed): LAN probe discovery code reviewed and confirmed ready for device testing at the time. `useLanProbes` (two-layer mDNS + .local fallback, consecutive-fail IP-change eviction after 3 failures), `useZeroconfDiscovery` (8 s scan window, 24 h TTL persistence, `mdnsScanEmpty` proxy for iOS Local Network permission denial), and `LiveCookSection.tsx` (all five no-probe UX states: linkedButEmpty, lanScanEndedEmpty, noLinkedAccounts, anyProbeScanning, noneSelected) — all in good shape. **Hardware test matrix still needed a human to run on a real iOS/Android build** with physical WiFi probe hardware. (This third-party probe cloud integration path was subsequently removed.)
...
- **2026-06-06 — Builds #113–#116 / v1.0.17**: third-party probe cloud integrations (later removed) — in-app password reset and inline error feedback; nav freeze fix (dismiss AI modal before router.push); probe empty-state layout fix.
